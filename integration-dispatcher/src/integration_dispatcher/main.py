...
async def _integration_dispatcher_shutdown() -> None:
    from shared_clients import close_zammad_rest_service_client

    await close_zammad_rest_service_client()
...
def lifespan(app: FastAPI) -> Any:
    return create_shared_lifespan(
        ...
        custom_shutdown=_integration_dispatcher_shutdown,
    )
...
async def handle_zammad_webhook(
    ...
    """Zammad Manage → Triggers → Webhook POST /zammad/webhook."""
    try:
        body = await request.body()
